...
namespace KeelteKooli.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult EditTeacher(Teacher teacher, string NewPassword)
        {
            ...
            {
                var user = db.Users.Find(dbTeacher.ApplicationUserId);
                if (user != null)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(NewPassword))
                    {
                        ...
                        var passResult = UserManager.ResetPassword(user.Id, token, NewPassword);

                        if (!passResult.Succeeded)
                        {
                            ModelState.AddModelError("", string.Join(", ", passResult.Errors));
                            return View("Opetaja/Edit", teacher);
                        }
                    }
                }
